...
- The "NovAUltimateT2I" all-in-one node is designed to replace all other nodes used in a standard text-to-image workflow (including Krea2T-Enhancer-Advanced for K2T model, and ModelSamplingAuraFlow for ZiT & Flux model). It is already optimized to reload only the parts of the workflow that have actually changed into RAM, so it performs similarly to a standard workflow with multiple nodes. It is still under development, but you can try it out nonetheless; notably, this node requires only a single "spaghetti" connection to link it to the final image preview.
